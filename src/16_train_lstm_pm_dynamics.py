...
class LSTMPMRegressor(nn.Module):
    def __init__(
        self,
        input_dim: int,
        output_dim: int,
...
) -> Tuple[LSTMPMRegressor, Dict[str, List[float]], np.ndarray]:
    model = LSTMPMRegressor(
        input_dim=input_dim,
...
def plot_loss(histories: Dict[int, Dict[str, List[float]]], output_path: Path) -> None:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE loss")
    plt.title("LSTM PM dynamics training curves")
...
def plot_r2(agg_df: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.xlabel("Mean R²")
    plt.title("LSTM PM dynamics: GroupKFold R² by target")
    plt.tight_layout()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_path, dpi=200)
    plt.close()
...
def write_report(
    ...
    lines = []
    lines.append("# LSTM PM Dynamics Report")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "This experiment adds a sequence-based deep learning model to the project. "
        "The model receives a local sequence of neighboring EEG/POW windows and predicts "
        "the PM metrics for the selected target position. This directly tests whether "
        "local temporal dynamics improve cognitive-state prediction compared with "
        "single-window tabular baselines."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", default="lstm_pm_dynamics", type=str)
    parser.add_argument(
        "--feature-set",
...
def main() -> None:
    ...
    print("=" * 80)
    print("Train LSTM PM dynamics model")
    print("=" * 80)
    print(f"Root: {root}")
